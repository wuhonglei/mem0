...
| `GET` | `/memories` | Get all memories (filter by `user_id`, `agent_id`, or `run_id`; `latest_only`, `include_merged`) |
...
| `POST` | `/search` | Search memories (`latest_only`, `include_merged`) |
| `POST` | `/dream` | Run a merge / supersede / synthesis pass for a user |
| `GET` | `/dream` | List recent Dream passes |
| `GET` | `/dream/{pass_id}` | Get one Dream pass and its actions |
